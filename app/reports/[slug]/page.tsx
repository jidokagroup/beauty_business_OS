...
export const metadata: Metadata = { title: "Report Detail - Luna & Sage Demo OS" };
  ...
  return (
    <section className="mb-6 rounded-xl border border-border bg-surface p-5">
      ...
      <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
        ...
        <label className="grid gap-1 text-xs uppercase tracking-[0.12em] text-text-muted">
          Location
          <select className={inputClass} defaultValue="Luna & Sage Studio">
            {["Luna & Sage Studio", "All locations"].map((option) => <option key={option}>{option}</option>)}
          </select>
        </label>
        <label className="grid gap-1 text-xs uppercase tracking-[0.12em] text-text-muted">
          Employee
          <select className={inputClass} defaultValue="All employees">
            {["All employees", "Avery", "Nova", "Jordan", "Riley"].map((option) => <option key={option}>{option}</option>)}
          </select>
        </label>
