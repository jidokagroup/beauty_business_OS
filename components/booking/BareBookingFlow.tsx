...
function providerOptions(service: typeof allServices[number] | undefined) {
  ...
  if (service.kind === "barber") return ["Jordan"];
  if (service.kind === "womensHair") return ["Riley"];
  if (service.kind === "lashExtensions" || service.kind === "lash") return ["Avery", "Nova"];
  if (["facial", "body", "brow", "waxing", "addon"].includes(service.kind)) return ["Avery"];
...
export default function BareBookingFlow() {
  ...
  const [artist, setArtist] = useState("Jordan");
  ...
  const timeSlots = artist === "Nova" ? NA_LASH_TIME_SLOTS : BARE_TIME_SLOTS;
  ...
  function validateProviderAvailability() {
    ...
    if (artist === "Nova" && selectedDate.getDay() !== 0) {
  ...
  useEffect(() => {
    ...
    setPreferredTime((availableProviders[0] === "Nova" ? NA_LASH_TIME_SLOTS : BARE_TIME_SLOTS)[0]);
  }, [serviceName, availableProviders]);

  useEffect(() => {
    const availableSlots = artist === "Nova" ? NA_LASH_TIME_SLOTS : BARE_TIME_SLOTS;
    ...
    return (
      <div className="rounded-xl border border-border bg-surface p-8 text-center">
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-text-secondary">
          Luna & Sage Studio has your appointment request. The studio will review availability and follow up to confirm your visit.
        </p>
        <button
          type="button"
          onClick={() => {
      ...
      <section className="rounded-xl border border-border bg-surface p-5">
        ...
        {isBarbering ? (
          <div className="mt-4 rounded-md border border-border bg-white p-5">
            <p className="font-serif text-2xl font-medium">Hmm, seems like Jordan is booked online.</p>
            <p className="mt-3 text-sm leading-relaxed text-text-secondary">
              For barbering, it is best to call or text Jordan directly at {BARE_STUDIOS.barberPhone}.
            </p>
          ...
          <div className="mt-4 rounded-md border border-border bg-white p-5">
            <p className="font-serif text-2xl font-medium">Women&apos;s hair books directly with Riley.</p>
            <p className="mt-3 text-sm leading-relaxed text-text-secondary">
              For women&apos;s hair appointments, it is best to call or text Riley directly at {BARE_STUDIOS.womensHairPhone}.
            </p>
            <div className="mt-5 flex flex-wrap gap-3">
          ...
          <>
            ...
            <p className="mt-3 text-center text-xs text-text-muted">Appointment requests are reviewed by Luna & Sage Studio before confirmation.</p>
          </>
        )}
      </section>
    </form>
